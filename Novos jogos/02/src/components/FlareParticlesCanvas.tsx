...
export const FlareParticlesCanvas: React.FC<FlareParticlesCanvasProps> = ({
  ...
  // Trigger miss burst when isMiss turns true
  useEffect(() => {
  ...
  // Trigger celebration burst when isPerfect turns true
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Spawn ongoing flare sparks while finger is pressed
      ...
      // Draw Main Flare Head directly at player finger position
      if (currentPos && isDragging) {
        const px = currentPos.x;
        const py = currentPos.y;

        ctx.save();
        // Big outer flare glow
