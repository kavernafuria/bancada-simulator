...
export const TacticalPitch: React.FC<TacticalPitchProps> = ({
  ...
  const ataX = Math.min(92, Math.max(20, pressure * 0.85));
  const pdX = Math.min(90, Math.max(25, pressure * 0.78));
  const peX = Math.min(90, Math.max(25, pressure * 0.78));
  const meiX = Math.min(75, Math.max(15, pressure * 0.62));
  ...
  return (
    <div
      className={`relative w-full rounded-2xl overflow-hidden border transition-all duration-300 flex flex-col justify-between shadow-2xl ${
        isAbove85
          ? "border-amber-400/80 bg-zinc-950 ring-2 ring-amber-500/50 shadow-amber-500/20"
          : "border-zinc-800 bg-zinc-950"
      }`}
    >
      {/* Top Header do Campinho */}
      <div className="relative z-20 px-3.5 py-2.5 bg-zinc-900/90 border-b border-zinc-800 backdrop-blur-md flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {isAbove85 ? (
            <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-amber-500 text-zinc-950 text-[10px] font-black uppercase tracking-wider animate-bounce shadow-md">
              <Flame className="w-3 h-3 text-red-700" />
              <span>Caldeirão (&gt;85%)</span>
            </div>
          ) : (
            <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-zinc-800 text-zinc-400 text-[10px] font-mono">
      ...
      <div className="relative w-full h-40 sm:h-52 lg:h-64 p-2 sm:p-3 bg-gradient-to-b from-emerald-950 via-emerald-900 to-emerald-950 overflow-hidden flex items-center justify-center select-none">
        ...
        {/* Efeito de calor sobre o campo quando > 85% */}
        {isAbove85 && (
          <div className="absolute inset-0 pointer-events-none bg-gradient-to-r from-transparent via-amber-500/30 to-rose-600/40 mix-blend-screen animate-pulse z-10 transition-opacity duration-300" />
        )}
        ...
        <div className="relative w-full h-full border-2 border-white/60 rounded-sm flex items-center justify-center">
          ...
          {/* Grande Área Adversária (Direita) */}
          <div
          ...
          <div
            className={`absolute right-0 inset-y-18 w-4 border-2 border-r-0 border-white rounded-l-xs flex items-center justify-center text-[8px] font-bold transition-colors ${
              isAbove85 ? "bg-amber-400/70 text-zinc-950 animate-pulse" : "bg-white/20 text-white"
            }`}
          >
            META
          </div>
          ...
          {/* Atacante Central (#9) */}
          <div
            className="absolute z-20 flex flex-col items-center pointer-events-none transition-all duration-500 ease-out"
            style={{ left: `${ataX}%`, top: "46%" }}
          >
            <div
              ...
              style={{ backgroundColor: homeTeam.primaryColor || "#000" }}
            >
              9
            </div>
          ...
          >
            <div
              ...
              style={{ backgroundColor: homeTeam.secondaryColor || "#f59e0b" }}
            >
              10
            </div>
          ...
          {/* Defensores Visitante (#3, #4) */}
          <div
            className="absolute z-15 flex flex-col items-center pointer-events-none transition-all duration-500 ease-out"
            style={{ left: `${isAbove85 ? 80 : 70}%`, top: "38%" }}
          >
            <div
              ...
              style={{ backgroundColor: awayTeam.primaryColor || "#dc2626" }}
            >
              3
            </div>
            <span className="text-[7px] text-zinc-300 drop-shadow">ZAG</span>
          </div>

          <div
            className="absolute z-15 flex flex-col items-center pointer-events-none transition-all duration-500 ease-out"
            style={{ left: `${isAbove85 ? 82 : 72}%`, top: "58%" }}
          >
            <div
              className="w-4.5 h-4.5 rounded-full border border-white/80 shadow-md flex items-center justify-center text-[8px] font-bold text-white"
              style={{ backgroundColor: awayTeam.primaryColor || "#dc2626" }}
          ...
          {/* Goleiro Rival */}
          <div className="absolute z-15 right-2 top-1/2 -translate-y-1/2 flex flex-col items-center pointer-events-none animate-pulse">
            <div className="w-5 h-5 rounded-full border-2 border-amber-300 bg-zinc-900 shadow-lg flex items-center justify-center text-[9px] font-black text-amber-300">
              🧤
            </div>
          ...
          <div
            ...
            style={{ left: `${ballCoords.x}%`, top: `${ballCoords.y}%` }}
          >
          ...
          {recentAttackEvent && (
            <div className="absolute inset-x-4 top-2 z-40 mx-auto max-w-xs px-3 py-1.5 bg-black/85 backdrop-blur-md rounded-full border border-amber-400/70 text-center shadow-xl flex items-center justify-center gap-1.5 animate-in zoom-in-95 duration-200">
      ...
      {/* Bottom Bar */}
      <div className="relative z-20 px-3.5 py-2.5 bg-zinc-950/95 border-t border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-2">
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <div className="text-[11px] font-bold text-zinc-300">
            Fase:{" "}
            <span className={isAbove85 ? "text-amber-400 font-black" : "text-zinc-200"}>
              {getPhaseText()}
            </span>
          </div>
        </div>
